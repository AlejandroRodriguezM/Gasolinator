...
public class FuncionesComboBox {
	...
	/**
	 * Crea y devuelve un objeto Evento con valores obtenidos de los ComboBoxes.
	 *
	 * @param totalComboboxes El número total de ComboBoxes.
	 * @param comboboxes      La lista de ComboBoxes que contienen los valores del
	 *                        Evento.
	 * @return Un objeto Evento con los valores seleccionados en los ComboBoxes.
	 */
	private Evento getEventoFromComboBoxes(List<ComboBox<String>> comboboxes) {
	 ...
	 * según el Evento proporcionado.
	 *
	 * @param totalComboboxes El número total de ComboBoxes.
	 * @param comboboxes      La lista de ComboBoxes.
	 * @param evento           El Evento que se utilizará como base para obtener los
	...
	public void actualizarComboBoxes(List<ComboBox<String>> comboboxes, Evento evento) {
		...
		if (!sql.isEmpty()) {
			...
			ListasEventosDAO.actualizarDatosAutoCompletado(sql);
			int cantidadDeComboBoxes = comboboxes.size();
			for (int i = 0; i < cantidadDeComboBoxes; i++) {
				comboboxes.get(i).hide();

				List<String> itemsActuales = ListasEventosDAO.listaOrdenada.get(i);
		...
		for (ComboBox<String> comboBox : comboboxes) {
			...
			comboBox.valueProperty().addListener((observable, oldValue, newValue) -> {
				...
				if (newValue == null || newValue.isEmpty()) {
					...
					modificarPopup(comboBox);
					Evento evento = getEventoFromComboBoxes(comboboxes);
					actualizarComboBoxes(comboboxes, evento);
				}
			});
		}
	}
	...
	public void rellenarComboBox(List<ComboBox<String>> comboboxes) {
		...
		for (int i = 0; i < comboboxes.size(); i++) {
			...
			List<String> items = ListasEventosDAO.itemsList.get(i);
	...
	private void handleComboBoxEvent(List<ComboBox<String>> comboboxes, ComboBox<String> comboBox, int index) {
		...
		List<String> currentItems = ListasEventosDAO.listaOrdenada.get(index);

		if (atLeastOneNotEmpty) {
			setupFilteredPopup(comboboxes, comboBox, currentItems);
		} else {
			setupFilteredPopup(comboboxes, comboBox, ListasEventosDAO.itemsList.get(index));
		}
	...
	private void setupFilteredPopup(List<ComboBox<String>> comboboxes, ComboBox<String> originalComboBox,
		...
		filterTextField.textProperty().addListener((observable, oldValue, newValue) -> {
			...
			if (newValue != null && !newValue.isEmpty()) {
				...
				List<String> newFilteredItems = ListasEventosDAO.itemsList.get(comboboxes.indexOf(originalComboBox))
				...
				List<String> allFilteredItems = new ArrayList<>(
						ListasEventosDAO.listaOrdenada.get(comboboxes.indexOf(originalComboBox)));
				currentFilteredItems.clear();
...
// Configurar el texto filtrado y solicitar el enfoque en el campo de texto
	...
	public static void rellenarComboBoxEstaticos(List<ComboBox<String>> comboboxes) {
		String[] valores = { "Marvel", "League of eventos", "Previews World", "Panini eventos" };
	...
	/**
	 * Funcion que permite modificar la puntuacion de un evento, siempre y cuando el
	...
	/**
	 * Funcion que permite seleccionar en el comboBox "numeroEvento" y lo devuelve,
	 * para la busqueda de evento
	 * 
	 * @return
	 */
	public static String numeroCombobox(ComboBox<String> numeroEvento) {
		String numEvento = "";

		if (numeroEvento.getSelectionModel().getSelectedItem() != null) {
			numEvento = numeroEvento.getSelectionModel().getSelectedItem();
		}
