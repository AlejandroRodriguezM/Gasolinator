...
public class Ventanas {
	...
	public void cerrarStages() {
		...
		cerrarStage(accionEvento);
		cerrarStage(opcionesAvanzadasStage);
		cerrarStage(cargaEventos);
...
//		});
	 ...
	 * la ventana actual.
	 */
	public static synchronized void verAccionEvento() {
		try {
			// Verifica si hay una ventana abierta y ciérrala si es necesario
			ventanaAbierta(accionEvento);

			FXMLLoader loader = new FXMLLoader(Ventanas.class.getResource("/ventanas/PantallaAccionEvento.fxml"));
			...
			Scene scene = new Scene(root);
			accionEvento = new Stage();
			accionEvento.setResizable(false);
			accionEvento.setTitle("Acciones comic"); // Titulo de la aplicación.

			accionEvento.getIcons().add(new Image("/Icono/icon2.png"));

			// Asocio el stage con el scene
			accionEvento.setScene(scene);
			accionEvento.show();

			// Indico que debe hacer al cerrar
			accionEvento.setOnCloseRequest(e -> {
				controlador.closeWindow();
				controlador.stop();
				accionEvento = null; // Establece la ventana actual a null cuando se cierra
			});
	 ...
	 */
	public void verCargaEventos(AtomicReference<CargaEventosController> cargaEventosControllerRef) {
		Platform.runLater(() -> {
			try {

				ventanaAbierta(cargaEventos);
				ventanaCerrada = false; // Marcar la ventana como cerrada
				FXMLLoader loader = new FXMLLoader(getClass().getResource("/ventanas/PantallaCargaEvento.fxml"));
				Parent root = loader.load();

				CargaEventosController cargaEventosController = loader.getController();
				cargaEventosControllerRef.set(cargaEventosController);
				...
				cargaEventos = stage;
				FuncionesManejoFront.getStageVentanas().add(cargaEventos);
				// Asocio el stage con el scene
				stage.setScene(scene);
				stage.show();

				stage.setOnCloseRequest(e -> {
					cargaEventosController.closeWindow();
	...
	public Stage devolverCargaEventos() {
		return cargaEventos;
	}
	...
	public void cerrarCargaEventos() {
		if (this.cargaEventos != null) {
			this.cargaEventos.close();
		}
	}

	public void cerrarVentanaAccion() {
		if (Ventanas.accionEvento != null) {
			Ventanas.accionEvento.close();
		}
	}
